...
</li>`).join("");h.insertAdjacentHTML("beforeend",t),B.refresh()}function O(){h.innerHTML=""}function v(){g.classList.remove("hidden")}function d(){g.classList.add("hidden")}function u(){y.classList.remove("hidden")}function p(){y.classList.add("hidden")}const w=document.querySelector("form"),m=w.querySelector('input[name="search-text"]'),R=document.querySelector(".js-load-more");let s=1,l="";const S=15;p();w.addEventListener("submit",E);async function E(r){if(r.preventDefault(),l=m.value.trim(),l===""){c.warning({message:"Please enter a search term before submitting.",position:"topRight"});return}m.value="",O(),s=1,u(),v();try{const t=await f(l,s);if(d(),t.hits.length===0){c.error({message:"Sorry, there are no images matching your search query. Please try again!",position:"topRight"});return}L(t.hits),t.totalHits>s*S&&u()}catch{c.error({message:"Failed to load images.Please try again.",position:"topRight"}),d()}}R.addEventListener("click",H);async function H(){s+=1,p(),v();try{const r=await f(l,s);d(),L(r.hits),s*S>=r.totalHits?(p(),c.info({message:"You have reached the end of the results"})):u();const i=document.querySelector(".gallery-item").getBoundingClientRect().height;window.scrollBy({left:0,top:i*2,behavior:"smooth"})}catch{c.error({message:"Failed to load more images.",position:"topRight"})}}
